...
<body>
	...
	<div class="container">
		...
		<div class="list-of-homecooks">

			<?php

				$query2 = "SELECT f.name,f.image AS food_img,f.cook_id,c.id AS cuisine_id,c.name AS cuisine_name, 
					r.name AS restaura_name,r.rating 
					FROM food f 
					INNER JOIN cuisines c ON f.cus_id = c.id
					INNER JOIN homecook r ON f.cook_id = r.id
				...
				while ($row = mysqli_fetch_assoc($result2)) {
					echo '<div class="retsurant-details">
							...
							<div class="info">
								<h2>'.$row['name'].'</h2>
								<a href="restaurant.php?id='.$row['cook_id'].'" style="text-decoration: none;"><h3>'.$row['restaura_name'].'</h3></a>
